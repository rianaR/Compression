...
for i=1:size(macroblock,3)
    ...
    [bestBlock_coords, movement] = bestCorrespondingBlock(block, coord_x, coord_y, imgd1, param_p);

    new_coord_x = coord_x - movement(2);
    new_coord_y = coord_y - movement(1);
    % todo : commenter tout ca
    % we crop the best imgd2block corresponding to imgd1
    block_temp = imgd1(bestBlock_coords(1):bestBlock_coords(1)+BLOCK_SIZE-1, bestBlock_coords(2):bestBlock_coords(2)+BLOCK_SIZE-1);
